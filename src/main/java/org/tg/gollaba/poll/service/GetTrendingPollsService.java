...
public class GetTrendingPollsService {
    ...
    public List<PollSummary> get(int limit){
        ...
        var pollIds = pollDailyStatsRepository.findTrendingPollIds(aggregationDate, limit);
        var polls = pollRepository.findAllById(pollIds);
        var sortedPolls = pollIds.stream()
            .map(id -> polls.stream()
