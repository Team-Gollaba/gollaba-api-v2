...
class GetDailyTopPollsControllerTest extends ControllerTestContext {
    private static final String TAG = Tags.POLL.tagName();
    private static final String DESCRIPTION = Tags.POLL.descriptionWith("일일통계 인기 투표 조회");

    @Autowired
    private GetPollsDailyPopularService getDailyTopPollsService;

    @Test
    void success(){
        when(getDailyTopPollsService.get(any(Pageable.class)))
